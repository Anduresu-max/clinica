    ...
    public partial class Eliminar : Form
    {
        ...
        private void iconButton2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Verificar si el paciente existe antes de eliminar los datos
                if (!CheckIfPacienteExists(nombrePaciente))
                {
                    MessageBox.Show("El paciente no existe en la base de datos.", "Información", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }

                // Eliminar datos de la tabla "datos_generales"
                string deleteDatosGeneralel = "DELETE FROM datos_generales WHERE nombre_paciente = @nombre_paciente";
                using (SQLiteCommand cmd = new SQLiteCommand(deleteDatosGeneralel, conn))
                {
                    cmd.Parameters.AddWithValue("@nombre_paciente", nombrePaciente);
                    cmd.ExecuteNonQuery();
                }
                ...
                // Limpia los controles después de eliminar los datos
                ClearControls();
            }
            catch (Exception ex)
            {
        ...
        private bool CheckIfPacienteExists(string nombrePaciente)
        {
            string query = "SELECT COUNT(*) FROM datos_generales WHERE nombre_paciente = @nombre_paciente";
            using (SQLiteCommand cmd = new SQLiteCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@nombre_paciente", nombrePaciente);
